...
<button onClick={onRefresh} className="p-2 rounded-xl hover:bg-slate-100" title="Aktualisieren">
...
<div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-rose-500" />{selectedDisplay}</div>
<div className="flex items-center gap-2"><CalendarIcon className="w-4 h-4" />{deLongFmt.format(new Date())}</div>
</div>
)}
</header>
);
